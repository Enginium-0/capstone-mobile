...
export async function postReservationRequest({ request = {} }) {
    ...
    try {
        equipments = await getEquipments({ byIds: request.equipments });
        console.log("🔧 Retrieved equipments:", equipments);

        departments = new Set(equipments.map(eq => eq?.department).filter(Boolean));
        departments.add('Facilities_Office');
        console.log("🏢 Departments involved:", Array.from(departments));
    } catch (err) {
        console.error("❌ Error fetching equipments or departments:", err);
    }
    ...
    try {
        if (request.ap?.uri) {
            console.log("📎 Uploading attached document...");
